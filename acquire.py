...
#***************************TIME PERMITTING, NOTES FOR COMING BACK TO SOFT ENCODE THE COMBINED STORES FUNCTIONS*******************************
'''
max_page ---> number of pages
>> requests.get(url).json()['payload']['max_page']
  << 3
***if max_page > 1, can run while loop to concat additional pages to df***
...
next_page ---> end url string for pages after 1, can add to base to get full url string from subsequent pages
>> request.get(url).json()['payload']['next_page']
  << '/api/v1/items?page=2'
                               .../api/v1/items?page=2'
